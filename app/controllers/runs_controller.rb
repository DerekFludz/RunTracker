class RunsController < ApplicationController
  ...
  def index
    ...
    @mile_count = Run.where(user_id: params[:user_id]).sum(:distance)
    ...
    @fast_pace = Run.minimum(:pace).to_s.gsub(".",":")
    @fast_pace_mil = Run.order(:pace).first
    @fast_pace_ind = Run.where(outdoors: false).minimum(:pace).to_s.gsub(".",":")
    @fast_pace_out = Run.where(outdoors: true).minimum(:pace).to_s.gsub(".",":")
